...
class Vylino_WA_Core {
    public function boot(){
        $repository=new Vylino_WA_Repository();$client=new Vylino_WA_WhatsApp_Client();$ai=new Vylino_WA_AI_Manager($repository);$router=new Vylino_WA_Router($repository,$client,$ai);$rest=new Vylino_WA_REST($router,$repository,$client);$admin=new Vylino_WA_Admin($repository);
        add_action('rest_api_init',array($rest,'register_routes'));$admin->register();add_action('vylino_wa_handoff_requested',array($this,'notify_handoff'),10,3);
    }
